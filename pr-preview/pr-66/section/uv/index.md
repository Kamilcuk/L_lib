...
1. **Running Stage**: Start the loop with `L_uv_run`. During this stage, handles can be added or removed dynamically from within callbacks.
...
1. **Running Stage**: After `L_uv_run` is called, functions like `L_uv_break`, `L_uv_poke`, and `L_uv_current_*` become meaningful as they interact with the active loop.
