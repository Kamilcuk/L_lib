...
1. **Atoms:** These are the final arguments that are passed to the command being executed. By default, `L_xargs` splits each Record into Atoms using shell-like quoting rules (split mode). Use `-Z` to treat each Record as a single, solid Atom.
