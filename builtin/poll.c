...
static short
parse_events (const char *s)
{
  short ev = 0;
  if (!s || !*s) return POLLIN;
  while (*s)
    {
      switch (*s)
	{
	case 'r': ev |= POLLIN; break;
	case 'w': ev |= POLLOUT; break;
	case 'p': ev |= POLLPRI; break;
	}
      s++;
    }
  return ev ? ev : POLLIN;
...
static char *
format_revents (short revents)
{
  static char buf[8];
  int p = 0;
  if (revents & POLLIN) buf[p++] = 'r';
  if (revents & POLLOUT) buf[p++] = 'w';
  if (revents & POLLPRI) buf[p++] = 'p';
  if (revents & POLLHUP) buf[p++] = 'h';
  if (revents & POLLERR) buf[p++] = 'e';
  if (revents & POLLNVAL) buf[p++] = 'n';
  buf[p] = '\0';
  return buf;
}
...
static int
poll_internal (WORD_LIST *list, int is_ppoll)
{
  char *ret_var = NULL;
  ...
  if (ret_var)
      ...
      if (v && !array_p (v))
	{
      ...
      if (!v) v = make_new_array_variable (ret_var);
      if (!v)
	{
...
int
poll_subcommand (WORD_LIST *list)
{
